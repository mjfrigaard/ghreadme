...
#' @export
spiral_horizon_plot <- function(commits,
                                repo = NULL,
                                date_begin = NULL,
                                date_end = NULL,
                                title = NULL) {

  for (pkg in c("spiralize", "ComplexHeatmap",
  ...
  # Clip extreme days so the horizon bands scale sensibly.
  ...
  lt <- spiralize::spiral_horizon(df$date, y, use_bars = TRUE)

  lgd <- spiralize::horizon_legend(
    lt,
