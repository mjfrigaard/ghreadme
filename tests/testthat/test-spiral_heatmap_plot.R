...
test_that("spiral_heatmap_plot totals match input row count", {
  skip_if_not_installed("spiralize")
  skip_if_not_installed("ComplexHeatmap")

  commits <- make_test_commits()
  df <- with_null_graphics(spiral_heatmap_plot(commits))
  expect_equal(sum(df$commits), nrow(commits))
})

test_that("spiral_heatmap_plot filters to a single repo", {
  skip_if_not_installed("spiralize")
  skip_if_not_installed("ComplexHeatmap")

  commits <- make_test_commits()
  df <- with_null_graphics(spiral_heatmap_plot(commits, repo = "repo-a"))
