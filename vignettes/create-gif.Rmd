---
title: "Create a GIF for your GitHub README"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Create a GIF for your GitHub README}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE
)
```

GitHub profile READMEs support animated GIFs, making it easy to show a short
introduction that plays automatically when someone visits your profile. This
vignette walks through using `who_am_i_gif()` to generate that GIF and embed
it in your README.

## Prerequisites

`who_am_i_gif()` requires two packages that are not installed with `ghreadme`
by default:

```{r install}
install.packages(c("gifski", "ggplot2"))
```

`ggplot2` renders each line of text as a plot frame, `ggsave()` writes the
frames to numbered PNGs in a temporary directory, and
[`gifski`](https://github.com/r-rust/gifski) assembles those PNGs into the
final GIF.

## Create the GIF

Call `who_am_i_gif()` with the same information you would pass to `who_am_i()`.
The `output` argument controls where the GIF is saved — set it to a path inside
your profile repository:

```{r create-gif}
library(ghreadme)

who_am_i_gif(
  name   = "Martin",
  likes  = "#rstats and data visualization.",
  learn  = "Shiny app development, Python, and Linux",
  work   = "R package development tools.",
  collab = "#rstats packages for data science.",
  output = "intro.gif"
)
```

The function prints each message as a cumulative frame on a dark background,
mimicking console output. After the last frame there is a 4-second hold before
the GIF loops, giving readers time to read the final line.

### Adjust timing and size

| Argument | Default | Effect |
|---|---|---|
| `frame_duration` | `1.5` | Seconds each new-line frame is shown |
| `width` | `800` | GIF width in pixels |
| `height` | `400` | GIF height in pixels |

```{r custom-timing}
who_am_i_gif(
  name           = "Martin",
  likes          = "#rstats and data visualization.",
  learn          = "Shiny app development, Python, and Linux",
  work           = "R package development tools.",
  collab         = "#rstats packages for data science.",
  output         = "intro.gif",
  frame_duration = 2,
  width          = 900,
  height         = 450
)
```

### Omit the connect message

Set `connect = FALSE` to drop the final *"Want to connect?"* line:

```{r no-connect}
who_am_i_gif(
  name    = "Martin",
  likes   = "#rstats and data visualization.",
  learn   = "Shiny app development, Python, and Linux",
  work    = "R package development tools.",
  collab  = "#rstats packages for data science.",
  connect = FALSE,
  output  = "intro.gif"
)
```

## Embed the GIF in your README

In your profile repository, reference the GIF at the top of `README.Rmd`
(before the badge links) so it plays when visitors land on your profile:

~~~
---
title: "Hi there 👋"
output: github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, out.width = "100%")
```

![](intro.gif)
~~~

Knit `README.Rmd` to regenerate `README.md`, then commit both files:

```bash
git add README.Rmd README.md intro.gif
git commit -m "update profile README"
git push
```

Your profile page will display the animated introduction automatically.

## Full workflow

```{r full-workflow}
library(ghreadme)

# 1. Generate the GIF into your profile repo
who_am_i_gif(
  name   = "Martin",
  likes   = "#rstats and data visualization.",
  learn   = "Shiny app development, Python, and Linux",
  work    = "R package development tools.",
  collab = "#rstats packages for data science.",
  output = "path/to/mjfrigaard/intro.gif"
)

# 2. Generate badge markdown for the README body
gh_badges(
  username = "mjfrigaard",
  badge    = c("details", "commit_lang", "stats"),
  theme    = "dark"
)

# 3. Generate Stack Overflow badge HTML
so_rep(username = "martin-frigaard", user_id = "4926446")
```
